...
void main() {
  ...
  group('M4.7 Home → MainActivity', () {
    ...
    test(
        'requestMainActivity() while displaying transitions to fallback, '
        'preserves config and clears error', () async {
      final state = AppState(
        store: ConfigStore(),
        client: _OkConfigClient(),
        healthCheck: _makeHealthCheck(pingOk: true),
      ...
      expect(state.phase, AppPhase.fallback);
      expect(state.config, isNotNull,
          reason: 'cached config must survive Home transition');
      expect(state.error, isNull,
          reason: 'Home is user-initiated, not an error');
      state.dispose();
    });
    ...
    test(
        'returnToDisplaying() while in fallback with cached config '
        'transitions to displaying', () async {
      final state = AppState(
        store: ConfigStore(),
        client: _OkConfigClient(),
        healthCheck: _makeHealthCheck(pingOk: true),
    ...
    test('returnToDisplaying() with no cached config is a no-op', () async {
      ...
      await state.returnToDisplaying();
      expect(state.phase, AppPhase.fallback,
          reason: 'no cached config means there is nothing to return to');
      state.dispose();
    });

    test('requestMainActivity() tears down the previous WsClient', () async {
      final state = AppState(
        store: ConfigStore(),
        client: _OkConfigClient(),
        healthCheck: _makeHealthCheck(pingOk: true),
